...
class Location extends Component {
  ...
  findGeoLocation() {
    ...
    const MAPQUEST_GEOCODING_API_URL =
      'https://www.mapquestapi.com/geocoding/v1/address';
    axios
      .get(
        `${MAPQUEST_GEOCODING_API_URL}?key=${process.env.REACT_APP_MAPQUEST_API_KEY}&location=${this.state.userLocation}`
      )
      .then(response => {
  ...
  render() {
    return (
      <div className="Location">
        <h2 className="Location__headline--color location__headline">
          Enter your location
        </h2>
        <input
          type="text"
          name="userLocation"
          className="Location__location-input"
        ...
        <button
          ...
          className="Location__location-btn"
        >
          Find a place to eat
        </button>
      </div>
